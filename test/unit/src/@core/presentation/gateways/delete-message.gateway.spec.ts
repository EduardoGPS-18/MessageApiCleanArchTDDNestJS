...
class DeleteMessageUseCaseStub implements DeleteMessageUseCaseI {
  execute = jest.fn();
}
...
const makeSut = (): SutTypes => {
  ...
  return { sut, deleteMessageUseCase, currentUserEntity, currentUserClient };
};

describe('Delete Message Gateway Suit', () => {
  beforeEach(() => {
    const { sut, currentUserClient, currentUserEntity, deleteMessageUseCase } =
      makeSut();
  });
  it('Should call usecase correctly', async () => {
    const { sut, deleteMessageUseCase, currentUserClient, currentUserEntity } =
      makeSut();
    jest.spyOn(deleteMessageUseCase, 'execute');
    await sut.handle(
      {
  ...
  it('Should emit event remove-message with messageId on body on result succceed', async () => {
    const { sut, currentUserClient, currentUserEntity } = makeSut();
    await sut.handle(
      {
        groupId: 'any_group_id',
        messageId: 'any_message_id',
